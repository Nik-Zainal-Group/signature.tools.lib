...
#' Plot Indels Signatures or Catalogues (89 channels)
#'
  ...
  for(i in 1:npages){
    ...
    if(!is.null(output_file)) {
      if(plottype=="jpg"){
        ...
        cairo_pdf(output_file,width = ncolumns*9,height = nplotrows*3+0.5,pointsize = 26)
      }
...
#' tab to Indels Catalogue (89 channels)
#' 
...
tabToIndelsCatalogue89 <- function(indels,
                                   ...
                                   genome.v){
  # 1. indel_classifier89
  # 2. indel_highspecific
  # 3. gen_catalogue89
  
  # checks
  requiredColumns <- c("Sample","chr","position","REF","ALT")
  missingCols <- setdiff(requiredColumns,colnames(indels))
  if(length(missingCols)>0){
    message("[error tabToIndelsCatalogue89] Missing input columns: ",paste(missingCols,collapse = ","))
    return(NULL)
  }
  ...
  tablecols <- colnames(indels_tab_classified_and_filtered)
  sample_col = which(tablecols=="Sample")
  catalogues <- indelsig.tools.lib::gen_catalogue89(muts_list = indels_tab_classified_and_filtered,
