...
signatureFit_pipeline <- function(catalogues=NULL,
  ...
  if(sum(specified_files)>1){
    ...
    message("[signatureFit_pipeline error] too many mutation file types specified: ",paste(names(specified_files)[specified_files],collapse = ","),". Please specify only one.")
    return(NULL)
  }else if(sum(specified_files)==0){
    # If no mutation files were used, then just make sure the catalgues var is not NULL
    if(is.null(catalogues)){
      message("[signatureFit_pipeline error] no mutation files nor catalogues file specified. Nothing to run the analysis on.")
      return(NULL)
    }
  }else{
    # now here we have exaclty one mutation file to use, so build the catalogue accordingly
    if(!is.null(SNV_vcf_files)){
      message("[signatureFit_pipeline info] reading ",length(SNV_vcf_files)," SNV vcf mutation files and building catalogues.")
      ...
      message("[signatureFit_pipeline info] reading ",length(SNV_tab_files)," SNV tab mutation files and building catalogues.")
      ...
      message("[signatureFit_pipeline info] reading ",length(DNV_vcf_files)," DNV vcf mutation files and building catalogues.")
      ...
      message("[signatureFit_pipeline info] reading ",length(DNV_tab_files)," DNV tab mutation files and building catalogues.")
      ...
      message("[signatureFit_pipeline info] reading ",length(SV_bedpe_files)," SV bedpe mutation files and building catalogues.")
    ...
    if(is.null(catalogues)){
      ...
      if(mtype_catalogues==mtype_mutations){
        ...
        message("[signatureFit_pipeline error] mutation files provided (",names(specified_files)[specified_files],
  ...
  if(!is.null(signatures)){
    ...
    if(mtype_catalogues!=mtype_signatures){
      message("[signatureFit_pipeline error] mutation files or catalogues provided have a mutation type (",
  ...
  if(!is.null(signatures)){
    if(is.null(rare_signatures) & fit_method=="FitMS"){
      message("[signatureFit_pipeline error] user selected FitMS and provided a signatures file, which will be used for the common signatures. ",
              "Please also provide a file with the rare signatures using the parameter rare_signatures_file.")
      return(returnObj)
    }
    if(!is.null(organ)){
      message("[signatureFit_pipeline warning] organ parameter ",organ," will be ignored because the user has specified signature files.")
    }
    if(!is.null(signature_version)){
      message("[signatureFit_pipeline warning] signature_version parameter ",signature_version," will be ignored because the user has specified signature files.")
    }
  }else{
    ...
    if(is.null(signature_version)){
      message("[signatureFit_pipeline warning] signature_version parameter unspecified, using latest RefSigv2.")
    ...
    if(fit_method=="FitMS"){
      if(signature_version!="RefSigv2" | mtype_catalogues!="subs" | is.null(organ)){
        message("[signatureFit_pipeline error] user selected FitMS and provided no signature files. ",
    ...
    if(signature_version=="RefSigv2"){
      if(!is.null(organ)){
        ...
        if(fit_method=="Fit"){
          if(mtype_catalogues %in% c("subs","DNV")){
            ...
            message("[signatureFit_pipeline warning] rearrangements RefSig mutational signatures only available in RefSigv1, ",
                    "switching to signature_version=RefSigv1")
            signatures <- getOrganSignatures(organ = organ,version = 1,typemut = mtype_catalogues)
          }else{
            message("[signatureFit_pipeline error] mutation type ",mtype_catalogues," not available for automatic selection of signatures. ",
          ...
          if(ncol(signatures)==0){
            message("[signatureFit_pipeline error] RefSigv2 signatures not available for mutation type ",mtype_catalogues,
                    " and organ ",organ,".")
            return(returnObj)
          }
        }
        ...
        message("[signatureFit_pipeline info] no organ provided, so using the reference signatures appropriate for the signature version and mutation type.")
      ...
      if(!is.null(organ)){
        ...
        if(fit_method=="Fit"){
          if(mtype_catalogues %in% c("subs","rearr")){
            ...
            message("[signatureFit_pipeline warning] DNV RefSig mutational signatures only available in RefSigv2, ",
          ...
          # need to check that the getOrganSignatures functions returned something
          if(ncol(signatures)==0){
            message("[signatureFit_pipeline error] RefSigv1 signatures not available for mutation type ",mtype_catalogues,
                    " and organ ",organ,".")
            return(returnObj)
          }
        }
        ...
        if(mtype_catalogues == c("subs")){
          ...
          message("[signatureFit_pipeline warning] DBS RefSig mutational signatures only available in RefSigv2, ",
                  "switching to signature_version=RefSigv2")
          signatures <- referenceSignaturesDBSv1.01
        }else if(mtype_catalogues == c("rearr")){
          signatures <- RefSigv1_rearr
      ...
      if(!is.null(organ)){
        # TODO
        message("[signatureFit_pipeline error] organ signatures selection for COSMICv2 signatures not implemented yet. ",
        ...
        if(mtype_catalogues == c("subs")){
          ...
          message("[signatureFit_pipeline error] using COSMICv2 signatures for ",mtype_catalogues," currenty not supported. ",
                  "Please provide your own signatures using the signatures_file parameter, and also rare_signatures_file if using FitMS.")
          return(returnObj)
        }
      ...
      if(!is.null(organ)){
        # TODO
        message("[signatureFit_pipeline error] organ signatures selection for COSMICv3.2 signatures not implemented yet. ",
                "Leave organ=NULL and select signatures manually with the signature_names parameter.")
        return(returnObj)
      ...
      message("[signatureFit_pipeline error] signature version ",signature_version," unrecognised.")
      return(returnObj)
    }
  }
  ...
  if(!is.null(signature_names)){
    if(fit_method=="Fit"){
      ...
      if(!all(signature_names %in% sigsToUseNames)){
        message("[signatureFit_pipeline error] using reference signatures for ",mtype_catalogues,", but some signature names provided ",
      ...
      message("[signatureFit_pipeline warning] using FitMS, parameter signature_names will be ignored.")
    }
  ...
  if(fit_method=="Fit"){
    ...
    message("[signatureFit_pipeline error] unknown fit_method ",fit_method,".")
    return(returnObj)
  }
