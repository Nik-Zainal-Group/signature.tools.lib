...
test_that("test fitMStoJSON(), no bootstrap", {
...
test_that("test fitMStoJSON(), with bootstrap", {
...
test_that("test writeFitResultsToJSON(), with bootstrap", {
  ...
  res <- FitMS(catalogues = rnd_matrix,
               ...
               threshold_percent = 0.1,
               threshold_p.value = 0.1)
  writeFitResultsToJSON(fitObj = res,filename = "testJSONexportFitMS.json")

  unlink("testJSONexportFitMS.json.zip")
  expect_true(TRUE)

})

test_that("test writeFitResultsToJSON(), with bootstrap and Gini thresholds", {
