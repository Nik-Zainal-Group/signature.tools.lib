...
test_that("test vcfToSNVcatalogue() to convert a VCF file with SNV into an SNV catalogue, hg38.", {
  
  testSNV_vcffile <- "test.sub.vcf.gz"
  
  expected_cat <- read.table(file="test2.snv_hg38.tab",sep = "\t",header = TRUE,stringsAsFactors = FALSE)
  
  res_cat <- vcfToSNVcatalogue(testSNV_vcffile,genome.v = "hg38")
